...
namespace GpuResourceUtil
{
    ...
    CComPtr<IDxcBlob> CreateShader6ByFile(
        std::wstring filename,
        std::string entryPointFunc,
        std::string shaderType
    )
    {
        ...
        std::wstring curEntryFuncName = StringToWstring(entryPointFunc);
        std::wstring curShaderTypeName = StringToWstring(shaderType);
        LPCWSTR pszArgs[] =
        {
           filename.c_str(),            // Optional shader source file name for error reporting
                                         // and for PIX shader source view.  
            L"-E", curEntryFuncName.c_str(),              // Entry point.
            L"-T", curShaderTypeName.c_str(),            // Target.
    ...
    )
    {
        ...
        if (computeShader == nullptr)
        {
            auto computeShader6 = CreateShader6ByFile(shaderName.c_str(), "CSMain", "cs_6_6");
            desc_out.CS = CD3DX12_SHADER_BYTECODE(computeShader6.p->GetBufferPointer(), computeShader6.p->GetBufferSize());
        }
